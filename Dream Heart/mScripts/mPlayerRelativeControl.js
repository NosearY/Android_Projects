...
public var mainCharacter : Transform;
//public var player : GameObject;
public var gameLogic : GameObject;
//public var mainAnimation : CharacterController;

public var idleAnimation : AnimationClip;
public var walkAnimation : AnimationClip;
public var runAnimation : AnimationClip;
public var jumpPoseAnimation : AnimationClip;
public var pickPoseAnimation : AnimationClip;
...
function Update()
{
	...
	if(character.velocity.sqrMagnitude < 0.1) {
		mainCharacter.animation.CrossFadeQueued(idleAnimation.name, 0.3, QueueMode.CompleteOthers);
	}else if(character.velocity.sqrMagnitude < 0.8) {
		mainCharacter.animation.CrossFade(walkAnimation.name, 0.3);
	}else{
		mainCharacter.animation.CrossFade(runAnimation.name, 0.3);
	}
	...
	if ( character.isGrounded )
	{
		...
		if ( rotateJoystick.tapCount == 2 )
		{
			...
			mainCharacter.animation[jumpPoseAnimation.name].wrapMode = WrapMode.ClampForever;
			mainCharacter.animation.CrossFade(jumpPoseAnimation.name);
...
function OnTriggerEnter(other : Collider){ 
	...
	gameLogic.SendMessage("OnEnemeyEnter", other);
		
}

function OnTriggerExit(other : Collider){
	gameLogic.SendMessage("OnEnemeyExit", other);
}